...
export function ExportModal({ onClose }: { onClose: () => void }) {
  ...
  const issues = selectedLanguages.flatMap(language => canvases.flatMap((canvas, index) => {
    const message = slideExportIssue(canvas, language, currentLanguage);
  ...
  const count = canvases.length * selectedSizes.length * selectedLanguages.length;
  ...
  return createPortal(<div className={`${styles.workspace} ${styles.dialogBackdrop}`} role="presentation">
    <div className={styles.dialog} role="dialog" aria-modal="true" aria-label="Export screenshots" onKeyDown={event => { if (event.key === 'Escape' && !busy) onClose(); }}>
      <header><div><span className={styles.eyebrow}>READY FOR YOUR LISTING</span><h2>Export screenshots</h2></div><button autoFocus aria-label="Close export" disabled={busy} onClick={onClose}><IoCloseOutline /></button></header>
