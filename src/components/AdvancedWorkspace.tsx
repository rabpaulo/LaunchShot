...
export default function AdvancedWorkspace({ onClose }: { onClose: () => void }) {
  const { 
    canvases, 
    addCanvas, 
    globalSettings, 
  ...
  const handleCopyCurrent = async () => {
    ...
    setIsCopying(true);
    try {
      const success = await copyCanvasToClipboard((canvases.find(c => c.id === useEditorStore.getState().selectedCanvasId) || canvases[0]).id);
      if (success) {
      ...
      <main className={`flex-1 h-full overflow-hidden flex flex-col relative ${
        ...
        {!isPreviewMode && (
          ...
          <div className="flex items-center space-x-2.5 h-full">
            <button onClick={onClose} className="px-3 py-1.5 rounded-lg border text-xs font-semibold">Back to workspace</button>
